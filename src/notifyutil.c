...
usage(const char *name)
{
	...
	fprintf(stderr, "    --dump         dumps metadata to a file in current working directory (must be run as root)\n");
}
...
notifyutil_dump()
{
	...
	ret = notify_dump_status("notifyd.status");

	if(ret == NOTIFY_STATUS_OK)
	{
		fprintf(stdout, "Notifyd dump success! New file named notifyd.status created at current working directory\n");
	} else {
